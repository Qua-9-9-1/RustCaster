...
pub fn draw_coin(game_t: &mut Game, player: &Player, hit_x: i32, hit_y: i32, ray_nb: i32, angle_temp: f64)
{
...
pub fn draw_monster(game_t: &mut Game, player: &Player, hit_x: i32, hit_y: i32, ray_nb: i32, angle_temp: f64)
{
    let distance = f64::sqrt(f64::powi(hit_x as f64 - player.pos.x, 2) + f64::powi(hit_y as f64 - player.pos.y, 2));
    let vision_coefficient = f64::cos(player.angle - angle_temp);
    let wall_height = ((500.0 / (vision_coefficient * distance)) * 100.0) / 2.0;
...
pub fn draw_end(game_t: &mut Game, player: &Player, hit_x: i32, hit_y: i32, ray_nb: i32, angle_temp: f64)
{
    let distance = f64::sqrt(f64::powi(hit_x as f64 - player.pos.x, 2) + f64::powi(hit_y as f64 - player.pos.y, 2));
    let vision_coefficient = f64::cos(player.angle - angle_temp);
    let wall_height = ((500.0 / (vision_coefficient * distance)) * 100.0) / 2.0;
