...
pub fn draw_wall(game_t: &mut Game, player: &Player, hit_x: i32, hit_y: i32, ray_nb: i32, angle_temp: f64)
{
    ...
    let vision_coefficient = f64::cos((angle_temp - player.angle)); //> 0 as f64 { f64::cos(angle_temp - player.angle) } else { f64::cos(angle_temp - player.angle) * -1.0 };
    let wall_height = (((RAYS_LENGTH as f64 / distance) * 100.0)) as i32 / 2;
    let wall_start = (SCREEN_HEIGHT / 2) - (wall_height / 2) + 40;
    let wall_end = (((SCREEN_HEIGHT / 2) + (wall_height / 2)) + 40) as u32;
    ...
    let mut map_hit_x = (hit_x / BLOCK_SIZE) as usize;
    let mut map_hit_y = (hit_y / BLOCK_SIZE) as usize;

    println!("hit_x: {}, hit_y: {} distance: {} ray {}, wall height {}, vision coef {}", hit_x, hit_y, distance, ray_nb, wall_height, vision_coefficient);
    if game_t.environnement.outside == true {
        if (map_hit_x == 0 || map_hit_x == game_t.map_width - 1) || (map_hit_y == 0 || map_hit_y == game_t.map_height - 1)
